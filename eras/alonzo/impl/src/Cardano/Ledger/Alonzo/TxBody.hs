...
viewCompactTxOut (TxOutCompact' addr val) = (addr, val, SNothing)
viewCompactTxOut (TxOutCompactDH' addr val dh) = (addr, val, SJust dh)
viewCompactTxOut (TxOut_AddrHash28_AdaOnly stakeRef a b c d adaVal) =
  (compactAddr addr, fromJust (toCompact val), SNothing)
  where
    (addr, val) = viewAddrHash28_AdaOnly (Proxy @era) stakeRef a b c d adaVal
viewCompactTxOut (TxOut_AddrHash28_AdaOnly_DataHash32 stakeRef a b c d adaVal e f g h) =
  (compactAddr addr, fromJust (toCompact val), SJust (decodeDataHash32 e f g h))
  where
    (addr, val) = viewAddrHash28_AdaOnly (Proxy @era) stakeRef a b c d adaVal
