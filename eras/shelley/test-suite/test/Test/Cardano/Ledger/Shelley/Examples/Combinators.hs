...
import Control.SetAlgebra (eval, forwards, setSingleton, singleton, (∪), (◁), (⋪), (⋫))
...
newUTxO txb cs = cs {chainNes = nes'}
  where
    ...
    ptrs =  forwards . _ptrs . _dstate . _delegationState $ ls
    sd' = updateStakeDistribution @era (_stakeDistro utxoSt) utxoDel utxoAdd ptrs
